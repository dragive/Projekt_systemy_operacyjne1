...
/*@assgined to Kacper*/
char* split(char** input_line)
{
  char *returned=(*input_line);
  int i;
  for(i=0;(*input_line)[i]!=':' && (*input_line)[i]!='\n';i++);
  (*input_line)[i]='\0';
...
int check_line_format(char* input_line){
    char *info;
    int hours_temp = atoi(split(&input_line));
    int test=0;
    if(hours_temp>=0 && hours_temp<=23)
    {
        int minutes_temp = atoi(split(&input_line));
        if(minutes_temp>=0 && minutes_temp<=59)
        {
            char* command=split(&input_line);
            if(command[0]!='\0')
            {
              int info=atoi(split(&input_line));
              if(info>=0 && info<=2)
              {
                return 0;
              }
